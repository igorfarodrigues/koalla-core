...
class UserService(
    ...
    fun findByWaId(waId: String): User? = userRepository.findByWaId(waId)?.toDomain()

    fun findById(id: UUID): User? = userRepository.findById(id).orElse(null)?.toDomain()
    ...
    @Transactional
    fun getOrCreateByWaId(
        waId: String,
        fullName: String? = null,
    ): User = findByWaId(waId) ?: createUser(waId, fullName)
